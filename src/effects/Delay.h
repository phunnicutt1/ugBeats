...
public:
    Delay(const std::string& name = "Delay");
    ~Delay();
     ...
     * 
     * @param mix Mix ratio (0 = fully dry, 1 = fully wet)
     */
    void setMix(float mix);
    
    /**
     * @brief Get the current mix ratio
     * 
     * @return Current mix ratio
     */
    float getMix() const;
...
protected:
     ...
     */
    void processSampleStereo(float leftSample, float rightSample, float* leftOutput, float* rightOutput);
     ...
     */
    void processBufferStereo(float* leftBuffer, float* rightBuffer, int numSamples);
