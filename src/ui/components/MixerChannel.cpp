...
void MixerChannel::setNumSends(int numSends)
{
    ...
    {
        // Create a new SendControl and add it to the vector
        sends.emplace_back();
        
        // Get a reference to the newly added SendControl
        auto& send = sends.back();
        ...
        // Set up label
        send.label->setText("FX " + juce::String(sends.size()), juce::dontSendNotification);
        send.label->setJustificationType(juce::Justification::centred);
        
        addAndMakeVisible(send.slider.get());
        addAndMakeVisible(send.label.get());
    }
...
void MixerChannel::createControls()
{
    ...
    boldFont = boldFont.withStyle(juce::Font::bold);
