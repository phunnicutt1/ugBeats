...
void PatternEditor::drawKeyboard(juce::Graphics& g)
{
    ...
    for (int note = minNote; note <= maxNote; ++note) {
        ...
        g.fillRect(0.0f, static_cast<float>(y), static_cast<float>(keyboardWidth - 1), static_cast<float>(noteHeight));
        
        // Draw key borders
        g.setColour(juce::Colours::grey);
        g.drawRect(0.0f, static_cast<float>(y), static_cast<float>(keyboardWidth - 1), static_cast<float>(noteHeight));
